...
const Appearances = ({ route }) => {
  ...
  const createReport = async () => {
    ...
    const q = query(collection(db, 'Appearances'), where('date', '==', dateString), where('course_id', '==', course_id));
    const querySnapshot = await getDocs(q);
        ...
        [
          { text: 'No', onPress: () => navigation.navigate('HomePage'), style: 'cancel' },
          {
            text: 'Yes', onPress: async () => {
              ...
              try {
                await Promise.all(appearancesData.map((data) => addDoc(collection(db, 'Appearances'), data)));
