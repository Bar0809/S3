...
const Appearances = ({ route }) => {
  ...
  const createReport = async () => {
    ...
    if (querySnapshot.size > 0) {
      Alert.alert(
        "הוספת דיווח ",
        "שימו לב, יש דיווח נראות בתאריך זה למקצוע זה. האם ברצונכם להמשיך?",
        [
          {
            text: "No",
          ...
          },
          {
            text: "Yes",
            onPress: async () => {
              const appearancesData = students.map((student, i) => {
                return {
                  course_id: course_id,
                  class_id: classId,
                  date: startDateTime,
                  appearances: selectedValues[student.id],
                  s_id: student.id,
                  note: freeText[i],
              ...
              try {
                await Promise.all(
                  appearancesData.map((data) =>
                    addDoc(collection(db, "Appearances"), data)
                  )
                );
                Alert.alert("", 'דו"ח הנראות הוגש בהצלחה!');
  ...
  return (
    ...
    <View style={styles.title}>
      <Text style={styles.pageTitle}>דיווח נראות  - {className}</Text>
    </View>
