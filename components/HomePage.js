...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const getClasses = async () => {
      try {
        ...
        classes.forEach((classInfo) => {
          CheckCurrentColor(classInfo.classID);
        });
      } catch {
        Alert.alert("אירעה שגיאה בלתי צפויה", error.message);
      }
    ...
    const CheckCurrentColor = async (classID) => {
      try {
        ...
        colorsQuerySnapshot.forEach((doc) => {
          if (doc.data().class_color) {
            setCurrentColor((prevColors) => [
              ...prevColors,
              { classID, color: doc.data().class_color },
            ]);
          }
        });
      } catch (error) {
        Alert.alert("שגיאה לא צפויה", error.message);
      }
    };
  ...
  const updateColor = async (
    ...
    if (2.33 <= average && average >= 3) {
      color = "red";
    }

    const colorForClass = currentColor.find(
      (item) => item.classID === classID
    )?.color;

    if (colorForClass !== color) {
      const q = query(
        collection(db, "Colors"),
        where("t_id", "==", auth.currentUser.uid),
        where("class_id", "==", classID)
