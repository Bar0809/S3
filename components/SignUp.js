...
const handleSignUp = async () => {
  ...
  setFirstNameError(firstNameIsValid ? null : 'שם פרטי צריך להכיל לפחות 2 תווים');
  setLastNameError(lastNameIsValid ? null : 'שם משפחה צריך להכיל לפחות 2 תווים');
  setSchoolNameError(schoolNameIsValid ? null : 'שם ביהס צריך להכיל לפחות 2 תווים');
  ...
  return (
      ...
      <View style={styles.all}>
        ...
        <TextInput style={[styles.input, { textAlign: 'right', marginBottom: 20 }]} placeholder='  שם פרטי:' value={firstName} onChangeText={text => setFirstName(text)}></TextInput>
        {firstNameError && <Text style={styles.errorMessage}>{firstNameError}</Text>}
        <TextInput style={[styles.input, { textAlign: 'right', marginBottom: 20 }]} placeholder=' שם משפחה:' value={lastName}  onChangeText={text => setLastName(text)}></TextInput>
        {lastNameError && <Text style={styles.errorMessage}>{lastNameError}</Text>}
        <TextInput style={[styles.input, { textAlign: 'right', marginBottom: 20 }]} placeholder=' שם ביה"ס:' value={schoolName}  onChangeText={text => setSchoolName(text)}></TextInput>
        {schoolNameError && <Text style={styles.errorMessage}>{schoolNameError}</Text>}
