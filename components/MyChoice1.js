...
const MyChoice1 = ({ route }) => {
  ...
  useEffect(() => {
    ...
    const getType = async () => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        setNameOfCategory(data.myChoice1);
      });
    };
  ...
  const createReport = async () => {
    if (icons1 === true) {
      ...
      if (querySnapshot.size > 0) {
        Alert.alert(
          "הוספת דיווח ",
          "שימו לב, יש דיווח בתאריך זה למקצוע זה. האם ברצונכם להמשיך?",
          [
            {
              text: "לא",
                ...
                try {
                  await Promise.all(
                    myChoiceData.map((data) =>
                      addDoc(collection(db, "MyChoice1"), data)
                    )
                  );
  ...
  <Text style={styles.pageTitle}> דיווח {nameOfCategory}</Text>
</View>
...
<Text style={[styles.pageTitle , {color:'black' ,fontSize: 22,lineHeight: 14,}]}>{className} - {courseName}</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} horizontal={false}>
        ...
        {icons1 ? (
          ...
          <View style={styles.tableContainer}>
            <View style={styles.tableHeader}>
              <Text style={styles.tableHeaderText}>טקסט חופשי</Text>
              <Text
                style={{ textAlign: "right", fontWeight: "bold", fontSize: 16 }}
              >
                שם התלמיד/ה
