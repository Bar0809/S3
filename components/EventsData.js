  ...
  const generateExcel = async () => {
    ...
    for (let i = 0; i < datesForExcel.length; i++) {
      ...
      const sheetName = `${day}.${month}.${year}`;
        ...
        const dietRef = collection(db, "Events");
        const q = query(
          dietRef,
          where("t_id", "==", auth.currentUser.uid),
        ...
        querySnapshot.forEach((doc) => {
          ...
          let eventType_ = "";
          if (eventType === "Verbal violence") {
            eventType_ = "אלימות מילולית";
          }
          else if(eventType === "physical violence"){
            eventType_ = "אלימות פיזית";
          }
           else if(eventType === "associateHonors"){
            eventType_ = "הצטיינות חברתית";
          } 
          else if(eventType === "Academic excellence"){
            eventType_ = "הצטיינות לימודית";
          }
          ...
          sheetData[courseName].push([studentName, eventType_, note]);
        });
        ...
        const uri = FileSystem.cacheDirectory + "אירועים_שונים.xlsx";
