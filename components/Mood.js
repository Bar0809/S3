...
const Mood = ({ route }) => {
  ...
  const createReport = async () => {
    ...
    if (querySnapshot.size > 0) {
      Alert.alert(
        "הוספת דיווח ",
        "שימו לב, יש דיווח מצב רוח בתאריך זה למקצוע זה. האם ברצונכם להמשיך?",
        ...
        [
          ...
          },
          {
            text: "כן",
            onPress: async () => {
              const moodData = students.map((student, i) => {
                return {
                  course_id: course_id,
                  class_id: classId,
                  date: startDateTime,
                  mode: selectedValues[student.id],
                  s_id: student.id,
                  note: freeText[i],
              ...
              try {
                await Promise.all(
                  moodData.map((data) => addDoc(collection(db, "Mood"), data))
                );

                Alert.alert("", 'דו"ח מצב הרוח הוגש בהצלחה!');
      ...
      const moodData = students.map((student, i) => {
        return {
          ...
          class_id: classId,
          date: startDateTime,
          mood: selectedValues[student.id],
          s_id: student.id,
          note: freeText[i],
          t_id: auth.currentUser.uid,
          class_name: className,
  ...
  return (
    ...
    <View style={styles.title}>
      <Text style={styles.pageTitle}>דיווח מצב רוח - {className}</Text>
    </View>
  ...
  {students.map((item, index) => (
    <View style={styles.studentRow} key={item.id}>
