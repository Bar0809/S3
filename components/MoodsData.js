...
const MoodsData = ({ route }) => {
  ...
  const check = async (clickedDate) => {
    ...
    try {
      ...
      const result = mediumStudentsNames.reduce((acc, [student, courseName, note]) => {
        if (!acc[courseName]) {
          acc[courseName] = {
            students: [],
            percent: 0,
      ...
      Object.keys(result).forEach((courseName) => {
        const mediumIcons = result[courseName].students.length;
        const percent = (mediumIcons * 100) / num;
    ...
    try {
      ...
      const result1 = sadStudentsNames.reduce((acc, [student, courseName, note]) => {
        if (!acc[courseName]) {
          acc[courseName] = {
            students: [],
            percent: 0,
  ...
  const generateExcel = async () => {
    ...
    for (let i = 0; i < datesForExcel.length; i++) {
      ...
      if (category === "Mood") {
        const dietRef = collection(db, "Mood");
        const q = query(
          dietRef,
          where("t_id", "==", auth.currentUser.uid),
        ...
        querySnapshot.forEach((doc) => {
          ...
          sheetData[courseName].push([studentName, mood_, note]);
        });
        ...
        const uri = FileSystem.cacheDirectory + "מצב_רוח.xlsx";
        ...
        const dietRef = collection(db, "FriendStatus");
        const q = query(
          dietRef,
          where("t_id", "==", auth.currentUser.uid),
          where("date", "==", presenceValue),
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (friendStatus === "good") {
            friendStatus_ = "הערכה חיובית";
          }
          else if(friendStatus === "sad"){
            friendStatus_ = "הערכה שלילית";

          } else {
            friendStatus_ = "הערכה בינונית";
          }
          ...
          sheetData[courseName].push([studentName, friendStatus_, note]);
        });
        ...
        const wbout = XLSX.write(wb, { type: "base64", bookType: "xlsx" });
        const uri = FileSystem.cacheDirectory + "מצב_חברתי.xlsx";
...
{category === 'FriendStatus' && (
                    ...
                    <View style={{alignItems:'center'}}>
                      ...
                      {Object.entries(mediumResult).map(
                        ([courseName, courseData]) => (
                          ...
                          >
                            {courseData.percent}% תלמידים קיבלו הערכה בינונית
                          </Text>
                          </View>
                        )
                      )}
 ...
 <Text
                          ...
                          >
                            {courseData.percent}% תלמידים קיבלו הערכה שלילית
                          </Text>
