...
const Scores = ({ route }) => {
  ...
  const createReport = async () => {
    ...
    const q = query(collection(db, 'Scores'), where('date', '==', dateString), where('course_id', '==', course_id));
    const querySnapshot = await getDocs(q);
        ...
        [
          { text: 'No', onPress: () => navigation.navigate('HomePage'), style: 'cancel' },
          {
            text: 'Yes', onPress: async () => {
              ...
              try {
                await Promise.all(scoresData.map((data) => addDoc(collection(db, 'Scores'), data)));
  ...
  return (
    <View>
      ...
      <View>
        <Text>תאריך</Text>
        <TextInput style={[styles.input, { textAlign: 'right' }]} value={dateString} onChangeText={handleChangeDate} placeholder="הכנס תאריך מהצורה (DD/MM/YYYY)" />
        {validDate ? (
      ...
      <Text>שם המטלה</Text>
      <TextInput style={[styles.input, { textAlign: 'right' }]} value={exerciseName} placeholder="הכנס/י שם המטלה/מבחן" onChangeText={handleChangeString} />


      <View>
        {/* <Text>בחר/י את סוג הציונים</Text> */}
{/* 
        <View >
          <RadioButton.Item
            value="string"
            status={type === "string" ? "checked" : "unchecked"}
            onPress={setType('string')} label='ציונים בביטויים ' />
          <RadioButton.Item
            value="numbers"
            status={type === "numbers" ? "checked" : "unchecked"}
            onPress={setType('numbers')} label='ציונים מספריים' />
        </View> */}
  ...
  renderItem={({ item, index }) => {
    ...
    return (
      <View style={styles.nameContainer}>
        ...
        <TextInput style={[styles.inputFreeText, { textAlign: 'right' }]} onChangeText={(text) => handleScoreChange(text, i)} value={scores[i]}></TextInput>
        <TextInput style={[styles.inputFreeText, { textAlign: 'right' }]} onChangeText={(text) => handleChangeText(text, i)} value={freeText[i]}></TextInput>
      </View>
    );
  }}
/>
