  ...
  const generateExcel = async () => {
    ...
    for (let i = 0; i < dates.length; i++) {
      ...
      querySnapshot.forEach((doc) => {
        ...
        const exerciseName = doc.data().exercise_name;
        const studentName = doc.data().student_name;
        const score = doc.data().score;
        const note = doc.data().note;
  
        const rowData = [studentName, score, note];
        if (!sheetData[courseName]) {
          sheetData[courseName] = {};
        }
        if (!sheetData[courseName][exerciseName]) {
          sheetData[courseName][exerciseName] = [];
        }
        sheetData[courseName][exerciseName].push(rowData);
      });
  
      const sheetNames = Object.keys(sheetData);
      sheetNames.forEach((courseName) => {
        const exercises = sheetData[courseName];
        const exerciseNames = Object.keys(exercises);
  
        exerciseNames.forEach((exerciseName) => {
          const exerciseData = exercises[exerciseName];
          const headerRow = ["תלמיד/ה", "ציון", "הערות"];
          const data = [headerRow, ...exerciseData];
          const ws = XLSX.utils.aoa_to_sheet(data);
          ws["A1"].s = boldCellStyle;
  
          const sheetTitle = `${courseName} - ${exerciseName} - ${sheetName}`;
    ...
    const uri = FileSystem.cacheDirectory + "ציונים.xlsx";
