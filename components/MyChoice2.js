...
const MyChoice2 = ({ route }) => {
  ...
  const createReport = async () => {
    ...
    if (icons1 === true) {
      ...
      const q = query(collection(db, 'MyChoice2'), where('date', '==', dateString), where('course_id', '==', course_id));
      const querySnapshot = await getDocs(q);
          ...
          [
            { text: 'No', onPress: () => navigation.navigate('HomePage'), style: 'cancel' },
            {
              text: 'Yes', onPress: async () => {
                ...
                try {
                  await Promise.all(myChoiceData.map((data) => addDoc(collection(db, 'MyChoice2'), data)));
